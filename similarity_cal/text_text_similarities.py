...
for index, row in tqdm(data.iterrows(), total=data.shape[0], desc="Processing images"):
    ...
    similarities = cosine_similarity(text_features.cpu().numpy())  # [num_texts, num_texts]
...
results_df.to_csv('/mnc/zwq/knowledge/similar_results/text_similarity_matrix.csv', index=True)
results_every.to_csv('/mnc/zwq/knowledge/similar_results/text_similarity_every.csv', index=False)
print('done')